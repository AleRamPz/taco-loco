...
function renderizarMenu() {
    ...
    state.menu.tacos.forEach((taco) => {
        ...
        <div class="product-card">
            <div onclick="abrirDetalleProducto('${cleanName}')" style="cursor:pointer;">
...
window.abrirDetalleProducto = function (nombreItem) {
    ...
    document.getElementById("pm-img").src = item.img || "";
    document.getElementById("pm-title").textContent = item.nombre;

    // Configuración de descripción directa de Google Sheets
    ...
    actualizarModalProductoHUD();
    document.getElementById("product-modal-overlay").classList.remove("hidden");
};

function actualizarModalProductoHUD() {
    if (!currentProductItem) return;
    document.getElementById("pm-qty").textContent = currentProductQty;
    document.getElementById("pm-price").textContent = `$${currentProductItem.precio.toFixed(2)}`;

    const total = currentProductItem.precio * currentProductQty;
    document.getElementById("pm-btn-add").textContent = `Agregar ${currentProductQty} al pedido - $${total.toFixed(2)}`;
}
...
function initCartEvents() {
    ...
    // ================== Eventos Bottom Sheet de Producto ==================
    const pModal = document.getElementById("product-modal-overlay");
    const pBtnCerrar = document.getElementById("close-product-modal");

    pBtnCerrar.addEventListener("click", () => pModal.classList.add("hidden"));
    pModal.addEventListener("click", (e) => {
        if (e.target === pModal) pModal.classList.add("hidden");
    });

    document.getElementById("pm-btn-minus").addEventListener("click", () => {
    ...
    document.getElementById("pm-btn-plus").addEventListener("click", () => {
        currentProductQty++;
        actualizarModalProductoHUD();
    });

    document.getElementById("pm-btn-add").addEventListener("click", () => {
        if (!currentProductItem) return;

        // Agregar 'n' cantidad al carrito global
        if (state.carrito[currentProductItem.nombre]) {
            state.carrito[currentProductItem.nombre].cant += currentProductQty;
        } else {
            state.carrito[currentProductItem.nombre] = { precio: currentProductItem.precio, cant: currentProductQty };
        }

        toastOk(`¡${currentProductQty} ${currentProductItem.nombre} agregado(s)!`);
        actualizarCarritoHUD(); // Esto también llama a actualizarBotonesMenu
