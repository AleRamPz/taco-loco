...
function renderizarMenu() {
    ...
    state.menu.tacos.forEach((taco) => {
        const itemInfoStr = encodeURIComponent(JSON.stringify(taco));
        htmlTacos += `
        <div class="product-card">
            ...
            <div>
                ...
                <div id="controls-${taco.nombre.replace(/\s/g, "")}" class="btn-wrap">
                    <button class="btn-primary flex-btn" onclick="agregarAlCarrito('${itemInfoStr.replace(/'/g, "%27")}')">Agregar al pedido</button>
                </div>
            </div>
        </div>`;
    });
    ...
    state.menu.bebidas.forEach((bebida) => {
        const itemInfoStr = encodeURIComponent(JSON.stringify(bebida));
        htmlBebidas += `
        <div class="product-card">
            <div>
...
// ==========================================
window.agregarAlCarrito = function (itemEnc) {
    const item = JSON.parse(decodeURIComponent(itemEnc));
    if (state.carrito[item.nombre]) {
        state.carrito[item.nombre].cant++;
    } else {
        state.carrito[item.nombre] = { precio: item.precio, cant: 1, fullItem: itemEnc };
    }
    ...
    [...state.menu.tacos, ...state.menu.bebidas].forEach(item => {
        ...
        if (cartItem && cartItem.cant > 0) {
            ...
            <div class="qty-controls">
                <button class="qty-btn" onclick="quitarDelCarrito('${item.nombre}')">－</button>
                <span class="qty-display">${cartItem.cant}</span>
                <button class="qty-btn" onclick="agregarAlCarrito('${cartItem.fullItem}')">＋</button>
            </div>
            `;
        } else {
            // Mostrar boton normal
            const itemEnc = encodeURIComponent(JSON.stringify(item)).replace(/'/g, "%27");
            cnt.innerHTML = `<button class="btn-primary flex-btn" style="padding:10px;" onclick="agregarAlCarrito('${itemEnc}')">Agregar al pedido</button>`;
        }
    });
