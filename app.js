            ...
            complete: function (results) {
                ...
                data.forEach(row => {
                    const obj = {
                        ...
                        disponible: row.Disponible?.trim().toUpperCase()
                    };
                    if (!obj.nombre || isNaN(obj.precio)) return; // Salto si fila vacía

                    // Si existe la columna 'Disponible' y dice explícitamente 'NO', saltamos el producto
                    if (obj.disponible === "NO" || obj.disponible === "FALSO") return;
