        ...
        <div class="product-card">
            ...
            <div>
                ...
                <div id="controls-${taco.nombre.replace(/\s/g, "")}" class="btn-wrap">
                    <button class="btn-primary flex-btn" onclick="agregarAlCarrito('${itemInfoStr}')">Agregar al pedido</button>
                </div>
            </div>
        </div>`;
    });
...
function actualizarBotonesMenu() {
    ...
    [...state.menu.tacos, ...state.menu.bebidas].forEach(item => {
        ...
        if (cartItem && cartItem.cant > 0) {
            ...
            cnt.innerHTML = `<button class="btn-primary flex-btn" style="padding:10px;" onclick="agregarAlCarrito('${item.fullItem}')">Agregar al pedido</button>`;
        }
    });
